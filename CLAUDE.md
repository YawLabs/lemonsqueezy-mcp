...
Releases run **locally** from a clean checkout of `main` via `./release.sh <version>`, which does steps 1-8 on the workstation -- lint, test, build, bump, commit, tag, push, npm publish, GitHub release, MCP Registry publish, verify. A workstation publish carries no provenance (`--provenance` is only passed in CI mode). Requires an npm AUTOMATION token in `~/.npmrc` and `gh auth login`, once per machine. Never `npm login --auth-type=web`: it overwrites the automation token with a 2FA-bound session and the next publish EOTPs. Idempotent; safe to re-run after partial failures.
