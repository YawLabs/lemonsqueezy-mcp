 ...
 * binary npm installed. It exists for the one host where that binary has been
 * measured unusable: on Windows ARM64 the native `@biomejs/cli-win32-arm64`
 * build crashes for SOME biome versions. Measured on that host: 2.4.16 runs
 * correctly, 2.5.4 exits 139 on any real check, and 2.5.13 runs correctly
 * again. The crash is a property of a particular arm64 build, NOT a permanent
 * defect of the architecture, and it is not caused by npm or by any shell
 * wrapper -- invoking the affected binary directly reproduces it.
 *
 * Nothing cheap distinguishes a good arm64 build from a bad one at runtime.
 * Measured: the broken 2.5.4 arm64 binary answers `--version` with exit 0 and
 * only dies (139) when asked to check real files, so a `--version` probe would
 * happily select a binary that is about to crash the gate. Rather than carry a
 * per-version allowlist that ages badly, this script routes Windows ARM64
 * through the x64 build OF THE SAME VERSION, which runs correctly under
 * Windows' x64 emulation. `YAWLABS_BIOME_NATIVE=1` forces the native binary
 * when you know the installed version's arm64 build is a good one.
 *
 ...
 *
 * Why it matters here specifically: this repo has no CI to fall back on --
 * there is no .github/workflows directory (.github holds only CODEOWNERS), so
 * no runner arbitrates formatting later. Whatever this script reports is the
 * ONLY lint signal that exists before `release.sh` publishes to npm from the
 * workstation.
 *
 ...
 * than a failed one, with no output to say why. This repo already bounds the
 * next unattended release step the same way: `npm test` (release.sh step 2)
...
/**
 * The biome version to provision: the version this repo actually INSTALLS,
 * read from package-lock.json and falling back to the installed package itself.
 *
 * It deliberately does NOT come from biome.json's `$schema`. That URL pins the
 * version the CONFIG is validated against -- an editor and schema concern --
 * which is not the same thing as the binary the lockfile installs, and the two
 * drift apart on any bump that touches one and not the other. That gap was the
 * bug this replaced: $schema said 2.4.12 while the lockfile installed 2.5.4, so
 * the gate provisioned and ran 2.4.12 and "lint clean" was a claim about a
 * version this repo does not use. Sourcing the version from the lockfile makes
 * the emulated binary the same version as the native one it stands in for.
 */
function installedBiomeVersion() {
  const lockPath = join(repoRoot, "package-lock.json");
  if (existsSync(lockPath)) {
    const lock = JSON.parse(readFileSync(lockPath, "utf8"));
    const locked = lock.packages && lock.packages["node_modules/@biomejs/biome"];
  ...
  throw new Error(
    "Could not determine which biome version this repo installs: package-lock.json has no\n" +
      'packages["node_modules/@biomejs/biome"].version entry, and node_modules/@biomejs/biome\n' +
      "is not installed either. Run `npm install` first, or set YAWLABS_BIOME_BIN=<path to a\n" +
      "working biome> to bypass version detection entirely.",
  );
}
 ...
 * done by another. A version-stamped path also means an install
...
if (crashed) {
  ...
  console.error(
    `[lint] biome ${how} (${binary}).\n` +
      "On Windows ARM64 some arm64 biome builds crash on a real check (2.5.4 measured at\n" +
      "exit 139); this script normally routes around that by running the x64 build of the\n" +
