...
- **A failed oam spawn under `auto` now says so** on stderr before falling back, instead of falling back silently. The fallback also survives it on an oam host, where the launcher pipes stdio and waits for the child's `close`: a spawn that fails still emits `close` (after `error`, with the negative errno), and that used to exit the launcher underneath the fallback it had just started. Piping, signal forwarding and the exit mirror now all wait for the child's `spawn` event. On a below-floor host the note no longer claims that no newer oam was found, since one was -- it is the one that failed to launch.
